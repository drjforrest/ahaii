...
class DataQualityReporter:
    ...
    def assess_data_quality(self, data: pd.DataFrame) -> List[DataQualityMetrics]:
        ...
        for (country_code, indicator_name), group in data.groupby(
            ...
            completeness_pct = (
                (available_years / total_years * 100) if total_years > 0 else 0
            )

            # Calculate average confidence
            valid_data = group[group["value"].notna()]
            avg_confidence = (
                valid_data["confidence_score"].mean() if len(valid_data) > 0 else 0
            )
            ...
            if len(valid_data) > 0:
                ...
                freshness_score = max(
                    0, 1 - (current_year - latest_year) / 5
                )  # Decay over 5 years
            else:
                freshness_score = 0
            ...
            if len(valid_data) > 2:
                ...
                if std_val > 0:
                    outliers = valid_data[
                        abs(valid_data["value"] - mean_val) > 2 * std_val
                    ]
                    outlier_count = len(outliers)

            # Calculate overall quality grade
            quality_score = (
                completeness_pct / 100 * 0.4
                + avg_confidence * 0.3
                + freshness_score * 0.2
                + max(0, 1 - outlier_count / available_years) * 0.1
    ...
    def generate_completeness_matrix(
    ...
    def identify_proxy_indicators(
        self, data: pd.DataFrame, quality_metrics: List[DataQualityMetrics]
    ) -> List[ProxyIndicatorSuggestion]:
        """
        ...
        for poor_indicator in poor_quality_indicators:
            ...
            if missing_indicator in self.PROXY_RELATIONSHIPS:
                for proxy_indicator, correlation in self.PROXY_RELATIONSHIPS[
                    missing_indicator
                ].items():
                    ...
                    proxy_quality = next(
                        (
                            ...
                            and m.indicator_name == proxy_indicator
                        ),
                        None,
                    )
                    ...
                    if (
                        proxy_quality
                        and proxy_quality.completeness_pct
                        > poor_indicator.completeness_pct + 20
                    ):
                        confidence_level = (
                            "High"
                            if correlation > 0.7
                            else "Medium" if correlation > 0.5 else "Low"
                        )
    ...
    def calculate_confidence_intervals(
        self, ahaii_results: List, data: pd.DataFrame
        ...
        for result in ahaii_results:
            ...
            if uncertainty_factors:
                uncertainty_margin = (
                    np.mean(uncertainty_factors) * 15
                )  # Max 15 point uncertainty
            else:
    ...
    def create_interactive_heatmap(self, completeness_matrix: pd.DataFrame) -> str:
        ...
        output_path = (
            self.output_dir
            / f"data_quality_heatmap_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        )
    ...
    def create_confidence_visualization(
        ...
        sorted_data = sorted(
            zip(countries, point_estimates, lower_bounds, upper_bounds),
            key=lambda x: x[1],
            reverse=True,
        )
        ...
        output_path = (
            self.output_dir
            / f"confidence_intervals_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        )
        fig.write_html(str(output_path))
    ...
    def compare_with_existing_indices(self, ahaii_results: List) -> Dict[str, Any]:
        ...
        for result in ahaii_results:
            ...
            if oxford_gari_scores.get(country_code):
                comparison_item["score_difference"] = (
                    result.total_score - oxford_gari_scores[country_code]
                )
        ...
        ahaii_ranks = {
            item["country_code"]: i + 1
            for i, item in enumerate(
                sorted(comparison_data, key=lambda x: x["ahaii_score"], reverse=True)
            )
        }

        oxford_ranks = {
            code: i + 1
            for i, (code, score) in enumerate(
                sorted(oxford_gari_scores.items(), key=lambda x: x[1], reverse=True)
            )
        }
    ...
    def generate_comprehensive_report(
        self, data: pd.DataFrame, ahaii_results: List
    ) -> str:
        """
        ...
        report = {
            ...
            "executive_summary": {
                "overall_data_quality": self._calculate_overall_quality_grade(
                    quality_metrics
                ),
                "best_coverage_country": self._find_best_coverage_country(
                    quality_metrics
                ),
                "most_complete_indicator": self._find_most_complete_indicator(
                    quality_metrics
                ),
                "key_data_gaps": self._identify_key_gaps(quality_metrics),
                "improvement_priorities": self._suggest_improvement_priorities(
                    quality_metrics
                ),
            },
            ...
            "recommendations": self._generate_data_improvement_recommendations(
                quality_metrics, proxy_suggestions
            ),
        }
        ...
        report_path = (
            self.output_dir
            / f"ahaii_data_quality_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def _calculate_overall_quality_grade(
        self, quality_metrics: List[DataQualityMetrics]
    ) -> str:
        """Calculate overall data quality grade across all metrics"""
        grade_scores = {"A": 4, "B": 3, "C": 2, "D": 1, "F": 0}
        total_score = sum(
            grade_scores[m.overall_quality_grade] for m in quality_metrics
        )
    ...
    def _find_best_coverage_country(
        self, quality_metrics: List[DataQualityMetrics]
    ) -> str:
        ...
        country_averages = {
            country: np.mean(scores) for country, scores in country_scores.items()
        }
        return max(country_averages.items(), key=lambda x: x[1])[0]

    def _find_most_complete_indicator(
        self, quality_metrics: List[DataQualityMetrics]
    ) -> str:
        ...
        indicator_averages = {
            indicator: np.mean(scores) for indicator, scores in indicator_scores.items()
        }
    ...
    def _identify_key_gaps(
        ...
        poor_quality = [
            m for m in quality_metrics if m.overall_quality_grade in ["D", "F"]
        ]
        gap_summary = {}
        ...
        return [
            f"{indicator} (affects {count} countries)"
            for indicator, count in sorted(
                gap_summary.items(), key=lambda x: x[1], reverse=True
            )[:5]
        ]

    def _suggest_improvement_priorities(
        self, quality_metrics: List[DataQualityMetrics]
    ) -> List[str]:
    ...
    def _generate_data_improvement_recommendations(
        self,
        quality_metrics: List[DataQualityMetrics],
        proxy_suggestions: List[ProxyIndicatorSuggestion],
    ) -> List[str]:
        ...
        low_coverage_indicators = [
            m.indicator_name for m in quality_metrics if m.completeness_pct < 50
        ]
        if low_coverage_indicators:
            recommendations.append(
                f"Priority data collection needed for: {', '.join(set(low_coverage_indicators)[:3])}"
            )

        # Proxy recommendations
        if proxy_suggestions:
            recommendations.append(
                f"Consider proxy indicators for {len(proxy_suggestions)} missing data points"
            )

        # Country-specific recommendations
        poor_coverage_countries = [
            m.country_name for m in quality_metrics if m.completeness_pct < 30
        ]
        if poor_coverage_countries:
            recommendations.append(
                f"Enhanced data collection partnerships needed for: {', '.join(set(poor_coverage_countries)[:3])}"
            )
