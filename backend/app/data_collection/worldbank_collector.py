...
class WorldBankCollector:
    ...
    def _check_cache(self, country_code: str, indicator_code: str, max_age_days: int = 7) -> Optional[pd.DataFrame]:
        """
        ...
        with sqlite3.connect(self.cache_db_path) as conn:
            ...
            df = pd.read_sql_query(query, conn, params=(country_code, indicator_code, cutoff_date))
    ...
    def _cache_data(self, data: pd.DataFrame, country_code: str, indicator_code: str):
        ...
        with sqlite3.connect(self.cache_db_path) as conn:
            # Insert/replace data
            data_to_cache.to_sql('indicator_data', conn, if_exists='replace', index=False, method='multi')
            
            # Update metadata
            success_rate = len(data_to_cache[data_to_cache['value'].notna()]) / len(data_to_cache) if len(data_to_cache) > 0 else 0
            
            conn.execute("""
                INSERT OR REPLACE INTO collection_metadata 
                (country_code, indicator_code, last_updated, success_rate, total_requests)
                VALUES (?, ?, ?, ?, ?)
            """, (country_code, indicator_code, datetime.now().isoformat(), success_rate, len(data_to_cache)))
    
    def fetch_indicator_data(self, country_code: str, indicator_code: str, 
        ...
        try:
            ...
            if len(data) < 2 or data[1] is None:
                logger.warning(f"No data returned for {indicator_code} in {country_code}")
                return self._create_empty_dataframe(country_code, indicator_code, start_year, end_year)
            ...
            logger.error(f"Error fetching data for {indicator_code} in {country_code}: {e}")
            return self._create_empty_dataframe(country_code, indicator_code, start_year, end_year)
    
    def _create_empty_dataframe(self, country_code: str, indicator_code: str, 
    ...
    def collect_all_indicators(self, start_year: int = 2020, end_year: int = 2023) -> pd.DataFrame:
        """
        ...
        for country_code in self.PILOT_COUNTRIES:
            ...
            for indicator_code, indicator_name in self.KEY_INDICATORS.items():
                ...
                logger.info(f"  Progress: {progress:.1f}% ({completed_requests}/{total_requests})")
        
        # Combine all data
        combined_df = pd.concat(all_data, ignore_index=True)
        
        # Save raw data
        output_path = self.cache_dir / f"worldbank_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    def generate_data_completeness_report(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        report = {
            'collection_summary': {
                ...
                'overall_completeness': len(data[data['value'].notna()]) / len(data) * 100
            },
            'by_country': {},
            'by_indicator': {},
            'by_year': {},
        ...
        for country in data['country_code'].unique():
            ...
            report['by_country'][country] = {
                ...
                'avg_confidence': country_data[country_data['value'].notna()]['confidence_score'].mean()
            }
        ...
        for indicator in data['indicator_code'].unique():
            ...
            report['by_indicator'][indicator] = {
                ...
                'available_data_points': available,
                'completeness_pct': (available / total * 100) if total > 0 else 0,
                'avg_confidence': indicator_data[indicator_data['value'].notna()]['confidence_score'].mean()
            }
        ...
        for year in sorted(data['year'].unique()):
            ...
            report['by_year'][year] = {
                ...
                'available_data_points': available,
                'completeness_pct': (available / total * 100) if total > 0 else 0,
                'avg_confidence': year_data[year_data['value'].notna()]['confidence_score'].mean()
            }
        ...
        if len(available_data) > 0:
            report['confidence_analysis'] = {
                ...
                'high_confidence_pct': len(available_data[available_data['confidence_score'] >= 0.8]) / len(available_data) * 100
            }
        
        # Save report
        report_path = self.cache_dir / f"data_completeness_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    print(f"Available data points: {report['collection_summary']['available_data_points']}")
    print(f"Overall completeness: {report['collection_summary']['overall_completeness']:.1f}%")
