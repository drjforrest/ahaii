...
class PolicyEvidence:
    ...
    source_url: str
    source_type: str  # 'government_website', 'oxford_gari', 'au_strategy', 'academic_paper'
    title: str
    relevant_text: str
    extraction_date: str
...
class PolicyIndicatorCollector:
    ...
    POLICY_INDICATORS = {
        'national_ai_strategy': {
            ...
            'keywords': ['artificial intelligence strategy', 'AI strategy', 'national AI policy', 'AI roadmap'],
            'weight': 0.30
        },
        'data_protection_regulation': {
            'description': 'Binary indicator for data protection/privacy legislation',
            'keywords': ['data protection act', 'privacy law', 'personal data protection', 'GDPR', 'data governance'],
            'weight': 0.25
        },
        'ai_ethics_guidelines': {
            'description': 'Binary indicator for AI ethics framework or guidelines',
            'keywords': ['AI ethics', 'artificial intelligence ethics', 'responsible AI', 'AI governance'],
            'weight': 0.20
        },
        'health_data_governance': {
            'description': 'Binary indicator for health data governance framework',
            'keywords': ['health data governance', 'medical data protection', 'health information privacy', 'EMR regulation'],
            'weight': 0.15
        },
        'digital_health_strategy': {
            'description': 'Binary indicator for digital health or eHealth strategy',
            'keywords': ['digital health strategy', 'eHealth strategy', 'health ICT policy', 'telemedicine policy'],
            'weight': 0.10
        }
    }
    ...
    def scrape_government_website(self, country_code: str, indicator_name: str) -> List[PolicyEvidence]:
        """
            ...
            try:
                ...
                for keyword in keywords:
                    if keyword.lower() in page_text:
                        ...
                        relevant_text = self._extract_relevant_context(page_text, keyword.lower())
    ...
    def _extract_relevant_context(self, text: str, keyword: str, context_words: int = 20) -> str:
    ...
    def search_african_union_strategy(self, country_code: str) -> List[PolicyEvidence]:
        ...
        for doc in au_documents:
            try:
                ...
                if country_code in ['ZAF', 'EGY', 'KEN']:  # Countries with more AI development
                    evidence = PolicyEvidence(
                        source_url=doc['url'],
    ...
    def validate_policy_indicator(self, country_code: str, indicator_name: str, 
                                 evidence_items: List[PolicyEvidence]) -> PolicyIndicator:
        """
        ...
        if total_evidence == 0:
            ...
            confidence_score = min(np.mean(confidence_scores) if confidence_scores else 0, 1.0)
    ...
    def collect_country_policy_indicators(self, country_code: str) -> List[PolicyIndicator]:
        """
        ...
        for indicator_name in self.POLICY_INDICATORS.keys():
            ...
            policy_indicator = self.validate_policy_indicator(country_code, indicator_name, evidence_items)
            ...
            logger.info(f"    {indicator_name}: {policy_indicator.indicator_value} (confidence: {policy_indicator.confidence_score})")
        
        return policy_indicators
    
    def _cache_evidence(self, country_code: str, indicator_name: str, evidence_items: List[PolicyEvidence]):
        ...
        with sqlite3.connect(self.cache_db_path) as conn:
            for evidence in evidence_items:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    country_code, indicator_name, evidence.source_url, evidence.source_type,
                    evidence.title, evidence.relevant_text, evidence.extraction_date, evidence.confidence_level
                ))
    ...
    def collect_all_countries(self) -> pd.DataFrame:
        ...
        for country_code in pilot_countries:
            try:
                country_indicators = self.collect_country_policy_indicators(country_code)
        ...
        # Export to CSV
        output_path = self.policy_cache_dir / f"policy_indicators_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        ...
        with sqlite3.connect(self.cache_db_path) as conn:
            for indicator in indicators:
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    indicator.country_code, indicator.indicator_name, indicator.indicator_value,
                    indicator.confidence_score, evidence_sources_json, indicator.last_updated,
                    indicator.expert_validated, indicator.validation_notes
                ))
    ...
    def generate_policy_matrix_report(self, policy_df: pd.DataFrame) -> str:
        ...
        report = {
            ...
            'summary_statistics': {
                ...
                'average_confidence_by_country': confidence_matrix.mean(axis=1).to_dict(),
                'average_confidence_by_indicator': confidence_matrix.mean(axis=0).to_dict()
            },
        ...
        # Save report
        report_path = self.policy_cache_dir / f"policy_matrix_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    print(f"Indicators per country: {len(policy_df) // policy_df['country_name'].nunique()}")
    
    print("\n=== By Country ===")
    country_summary = policy_df.groupby('country_name').agg({
        'indicator_value': 'sum',
        'confidence_score': 'mean'
    }).round(2)
    
    for country, stats in country_summary.iterrows():
        print(f"{country}: {stats['indicator_value']}/5 indicators, avg confidence: {stats['confidence_score']}")
