...
class AHAIIPilotReportGenerator:
    ...
    def compile_country_assessments(
        self,
        ...
        for result in enhanced_results:
            ...
            validation_summary = self._get_validation_summary(
                result.country_code, validation_results
            )
            data_quality_grade = self._get_data_quality_grade(
                result.country_code, data_quality_metrics
            )
            ...
            sorted_pillars = sorted(
                result.pillar_scores, key=lambda p: p.score, reverse=True
            )
            key_strengths = []

            for pillar in sorted_pillars[:2]:  # Top 2 pillars
                if pillar.score >= 60:
                    strength_area = self._pillar_to_strength_description(
            ...
            for pillar in sorted_pillars[-2:]:  # Bottom 2 pillars
                if pillar.score < 50:
                    gap_area = self._pillar_to_gap_description(
                        pillar.name, pillar.score
                    )
            ...
            ci_margin = (
                5 + (1 - result.overall_confidence) * 10
            )  # Larger margin for lower confidence
            confidence_interval = (
    ...
    def _get_validation_summary(
        self, country_code: str, validation_results: List
    ) -> str:
        """Get expert validation summary for country"""
        country_validations = [
            v
            for v in validation_results
            if hasattr(v, "country_code") and v.country_code == country_code
        ]
        ...
        validated_count = len(
            [v for v in country_validations if v.final_status.value == "validated"]
        )
    ...
    def _get_data_quality_grade(
        self, country_code: str, data_quality_metrics: List
    ) -> str:
        """Get data quality grade for country"""
        country_metrics = [
            m
            for m in data_quality_metrics
            if hasattr(m, "country_code") and m.country_code == country_code
        ]
        ...
        for pillar in result.pillar_scores:
            if pillar.score < 40:
                if pillar.name == "human_capital":
                    ...
                    roadmap.append(
                        "Priority: Create health AI innovation fund and incubator programs"
                    )

        return roadmap[:5]  # Top 5 recommendations

    def generate_regional_analysis(
        self, country_assessments: List[CountryAssessment]
    ) -> Dict[str, Any]:
        """
        ...
        regional_stats = {
            ...
            "tier_distribution": {
                "tier_1": len(
                    [a for a in country_assessments if a.tier_classification == 1]
                ),
                "tier_2": len(
                    [a for a in country_assessments if a.tier_classification == 2]
                ),
                "tier_3": len(
                    [a for a in country_assessments if a.tier_classification == 3]
                ),
            },
        }
        ...
        for pillar in pillars:
            pillar_scores = [
                assessment.pillar_breakdown[pillar]
            ...
            pillar_analysis[pillar] = {
                ...
                "top_performer": max(
                    country_assessments, key=lambda a: a.pillar_breakdown[pillar]
                ).country_name,
                "improvement_needed": [
                    a.country_name
                    for a in country_assessments
                    if a.pillar_breakdown[pillar] < 40
                ],
            }
        ...
        rankings = {
            "overall_ranking": [
                (a.country_name, a.final_score, a.regional_rank)
                for a in sorted(
                    country_assessments, key=lambda x: x.final_score, reverse=True
                )
            ],
            "pillar_leaders": {
                pillar: max(
    ...
    def _create_score_comparison_chart(
        self, assessments: List[CountryAssessment]
    ) -> go.Figure:
        """Create AHAII scores comparison with confidence intervals"""
        sorted_assessments = sorted(
            assessments, key=lambda a: a.final_score, reverse=True
        )
        ...
        fig.add_trace(
            go.Bar(
                ...
                marker_color=[
                    (
                        "#1f77b4"
                        if a.tier_classification == 1
                        else "#ff7f0e" if a.tier_classification == 2 else "#d62728"
                    )
    ...
    def _create_pillar_radar_chart(
        self, assessments: List[CountryAssessment]
    ) -> go.Figure:
    ...
    def _create_ranking_visualization(
        self, assessments: List[CountryAssessment]
    ) -> go.Figure:
    ...
    def _create_data_quality_heatmap(
        self, assessments: List[CountryAssessment]
    ) -> go.Figure:
        ...
        for assessment in assessments:
            ...
            row = [
                ...
                (assessment.confidence_interval[1] - assessment.confidence_interval[0])
                / 2
                * 10,  # Confidence width
            ]
        ...
        fig.update_layout(title="AHAII Data Quality Assessment by Country", height=400)
    ...
    def generate_final_report(
        ...
        visualization_paths = self.create_visualizations(
            country_assessments, regional_analysis
        )
        ...
        data_quality_assessment = {
            ...
            "country_grades": {
                a.country_name: a.data_quality_grade for a in country_assessments
            },
        ...
        expert_validation_summary = {
            ...
            "consensus_rate": (
                len(
                    [
                        v
                        for v in validation_results
                        if hasattr(v, "final_status")
                        and v.final_status.value == "validated"
                    ]
                )
            ...
            "country_validation_status": {
                a.country_name: a.expert_validation_status for a in country_assessments
            },
            "key_insights": [
        ...
        report_path = (
            self.reports_dir / f"ahaii_pilot_assessment_report_{timestamp}.json"
        )
            ...
            summary_data.append(
                {
                    ...
                    "Physical_Infrastructure": assessment.pillar_breakdown[
                        "physical_infrastructure"
                    ],
                    "Regulatory_Framework": assessment.pillar_breakdown[
                        "regulatory_framework"
                    ],
    ...
    def _calculate_overall_data_quality(self, data_quality_metrics: List) -> str:
        ...
        grades = [
            m.overall_quality_grade
            for m in data_quality_metrics
            if hasattr(m, "overall_quality_grade")
        ]
        grade_scores = {"A": 4, "B": 3, "C": 2, "D": 1, "F": 0}
    ...
    def _identify_critical_data_gaps(self, data_quality_metrics: List) -> List[str]:
        ...
        poor_quality_indicators = [
            ...
            for m in data_quality_metrics
            if hasattr(m, "overall_quality_grade")
            and m.overall_quality_grade in ["D", "F"]
        ]
    ...
    def _generate_executive_summary(
        self, report: PilotAssessmentReport, timestamp: str
    ) -> str:
        ...
        executive_summary = {
            ...
            "key_results": {
                ...
                "tier_distribution": report.regional_analysis["summary_statistics"][
                    "tier_distribution"
                ],
                "top_performer": report.country_assessments[0].country_name,
                "strongest_pillar": max(
                    report.regional_analysis["pillar_performance"].items(),
