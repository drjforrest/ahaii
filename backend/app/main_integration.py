...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("ahaii_pipeline.log"), logging.StreamHandler()],
)
...
class AHAIIIntegrationManager:
    ...
    def __init__(self, output_dir: str = "data"):
        ...
        self.policy_collector = PolicyIndicatorCollector(
            cache_dir=str(self.output_dir / "raw")
        )
        self.ecosystem_mapper = HealthAIEcosystemMapper(
            cache_dir=str(self.output_dir / "raw")
        )
        self.calculator = AHAIICalculator(
            output_dir=str(self.output_dir / "indicators")
        )
        self.enhanced_calculator = EnhancedAHAIICalculator(
            output_dir=str(self.output_dir / "indicators")
        )
        self.quality_reporter = DataQualityReporter(
            output_dir=str(self.output_dir / "processed")
        )
        self.validation_system = ExpertValidationSystem(
            validation_dir=str(self.output_dir / "processed")
        )
        self.report_generator = AHAIIPilotReportGenerator(
            output_dir=str(self.output_dir / "analysis" / "pilot_assessment")
        )
    ...
    def test_system_connectivity(self) -> Dict[str, bool]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"✗ World Bank API connectivity: FAILED (Status: {response.status_code})"
                )
        ...
        logger.info(
            f"Overall connectivity status: {'SUCCESS' if all_connected else 'PARTIAL/FAILED'}"
        )
    ...
    def run_data_collection_phase(self) -> Dict[str, Any]:
        ...
        try:
            ...
            policy_report = self.policy_collector.generate_policy_matrix_report(
                policy_data
            )
            ...
            logger.info(
                f"✓ Policy indicator collection: {len(policy_data)} indicators collected"
            )
        ...
        try:
            ...
            ecosystem_report = self.ecosystem_mapper.generate_ecosystem_report(
                org_data, ecosystem_metrics
            )
    ...
    def run_scoring_phase(self, collection_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✓ Basic scoring: {len(basic_results)} country scores calculated"
            )
            ...
            try:
                ...
                for country_code in pilot_countries:
                    if country_code in wb_data["country_code"].values:
                        ...
                        country_wb_data = wb_data[
                            wb_data["country_code"] == country_code
                        ]

                        enhanced_result = (
                            self.enhanced_calculator.calculate_enhanced_ahaii_score(
                ...
                enhanced_export_path = self.enhanced_calculator.export_results(
                ...
                logger.info(
                    f"✓ Enhanced scoring: {len(enhanced_results)} country scores calculated"
                )
    ...
    def run_validation_phase(
        self, collection_results: Dict[str, Any], scoring_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if wb_data is not None:
            try:
                ...
                quality_report_path = (
                    self.quality_reporter.generate_comprehensive_report(
                ...
                logger.info(
                    f"✓ Data quality assessment complete: {quality_report_path}"
                )
        ...
        if policy_data is not None and ecosystem_metrics is not None:
            try:
                ...
                expert_responses = self.validation_system.simulate_expert_responses(
                    validation_requests
                )
                ...
                logger.info(
                    f"✓ Expert validation complete: {len(consensus_results)} indicators validated"
                )
    ...
    def run_reporting_phase(
        ...
        if enhanced_results:
            ...
            logger.warning(
                "Final report generation skipped: no enhanced results available"
            )
            reporting_results["final_report"] = {
                "status": "skipped",
                "reason": "no_enhanced_results",
            }
    ...
    def run_complete_pipeline(self) -> Dict[str, Any]:
        ...
        results_path = (
            self.output_dir
            / f"ahaii_pipeline_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
