...
class AHAIICalculator:
    ...
    def normalize_indicator(self, value: float, indicator_name: str, confidence: float = 1.0) -> Tuple[float, float]:
        """
        ...
        if indicator_name not in self.NORMALIZATION_BOUNDS:
            logger.warning(f"No normalization bounds for {indicator_name}, using value as-is")
    ...
    def calculate_pillar_score(self, country_data: pd.DataFrame, pillar_name: str) -> PillarScore:
        """
        ...
        pillar_indicators = {
            indicator: name for indicator, name in self.INDICATOR_PILLAR_MAPPING.items()
            if name == pillar_name
        }
        ...
        for indicator in pillar_indicators:
            ...
            latest_data = indicator_data.dropna(subset=['value']).sort_values('year').tail(1)
    ...
    def generate_tier_classification(self, total_score: float, confidence: float) -> AHAIITier:
        """
        ...
        if confidence_adjusted_score >= 70 and confidence >= self.TIER_CONFIDENCE_THRESHOLDS[AHAIITier.IMPLEMENTATION_READY]:
            return AHAIITier.IMPLEMENTATION_READY
        elif confidence_adjusted_score >= 40 and confidence >= self.TIER_CONFIDENCE_THRESHOLDS[AHAIITier.FOUNDATION_BUILDING]:
            return AHAIITier.FOUNDATION_BUILDING
        else:
            return AHAIITier.DEVELOPMENT
    
    def generate_improvement_recommendations(self, pillar_scores: List[PillarScore], tier: AHAIITier) -> List[str]:
        """
        ...
        if tier == AHAIITier.DEVELOPMENT:
            recommendations.insert(0, "Focus on basic digital health infrastructure and workforce development")
        elif tier == AHAIITier.FOUNDATION_BUILDING:
            recommendations.insert(0, "Prioritize regulatory framework development and pilot AI implementations")
    ...
    def identify_data_quality_issues(self, country_data: pd.DataFrame) -> List[str]:
        ...
        if missing_indicators:
            issues.append(f"Missing data for {len(missing_indicators)} key indicators: {', '.join(list(missing_indicators)[:3])}")
        ...
        if len(low_confidence_data) > 0:
            issues.append(f"{len(low_confidence_data)} data points have low confidence scores")
        ...
        for indicator in available_indicators:
            ...
            if years_with_data < total_years * 0.5:
                issues.append(f"Incomplete time series for {indicator} ({years_with_data}/{total_years} years)")
        
        return issues
    
    def calculate_ahaii_score(self, country_data: pd.DataFrame, country_code: str, country_name: str) -> AHAIIResult:
        """
        ...
        logger.info(f"AHAII Score for {country_name}: {total_score:.1f} (Tier {tier.value})")
    ...
    def calculate_all_countries(self, data: pd.DataFrame) -> List[AHAIIResult]:
        ...
        for (country_code, country_name), country_data in countries:
            try:
                result = self.calculate_ahaii_score(country_data, country_code, country_name)
...
def main():
    ...
    for result in results:
        print(f"{result.country_name}: {result.total_score:.1f} (Tier {result.tier.value}, Rank {result.regional_rank})")
