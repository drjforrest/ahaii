...
class HealthAIInfrastructureClassifier:
    ...
    def __init__(self):
        ...
        self.economic_keywords = [
            ...
            "clinical AI adoption costs",
            "health AI reimbursement",
            "medical AI market",
            "healthcare technology economics",
            "digital health business model",
        ]

    def classify_infrastructure_relevance(
        ...
        scores = {
            "human_capital": self._calculate_keyword_relevance(
                text, self.human_capital_keywords
            ),
            "physical_infrastructure": self._calculate_keyword_relevance(
                text, self.physical_infrastructure_keywords
            ),
            "regulatory": self._calculate_keyword_relevance(
                text, self.regulatory_keywords
            ),
    ...
    def extract_infrastructure_signals(
        self, title: str, abstract: str
    ) -> List[Dict[str, Any]]:
            ...
            signals.append(
                {
                    ...
                    "confidence": 0.7,
                    "extracted_text": self._extract_relevant_sentence(
                        text, ["emr", "electronic medical record"]
                    ),
                }
            )
        ...
        if any(
            term in text
            for term in ["training program", "medical education", "clinical training"]
        ):
            signals.append(
                {
            ...
            signals.append(
                {
                    ...
                    "confidence": 0.8,
                    "extracted_text": self._extract_relevant_sentence(
                        text, ["infrastructure", "implementation"]
                    ),
                }
            )
...
class HealthAIAcademicScraper:
    ...
    def build_health_ai_search_query(
        self, source: str = "arxiv", max_results: int = 50, days_back: int = 30
    ) -> str:
    ...
    def _build_arxiv_query(self, max_results: int, days_back: int) -> str:
        ...
        keyword_query = " OR ".join(
            [f'all:"{term}"' for term in health_ai_terms[:8]]
        )  # Limit to avoid URL issues
    ...
    def _build_pubmed_query(self, max_results: int, days_back: int) -> str:
        ...
        query = (
            " AND ".join(search_terms[:3])
            + f" AND ({' OR '.join(self.african_countries[:10])})"
        )
    ...
    async def scrape_health_ai_papers(
        self, source: str = "arxiv", max_results: int = 50
    ) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Scraping {source} for health AI infrastructure papers: {max_results} results"
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully processed {len(processed_papers)} relevant health AI papers"
            )
    ...
    def process_health_ai_paper(
        ...
        try:
            ...
            pillar_scores = self.classifier.classify_infrastructure_relevance(
                title, abstract
            )
            health_ai_infra_score = sum(pillar_scores.values()) / len(pillar_scores)

            # Extract infrastructure signals
            infrastructure_signals = self.classifier.extract_infrastructure_signals(
                title, abstract
            )
    ...
    def calculate_african_relevance(
        ...
        for institution in self.african_institutions:
            if any(
                word in text for word in institution.lower().split()[:3]
            ):  # Check first 3 words
...
async def scrape_health_ai_infrastructure_papers(
    max_results: int = 50,
) -> List[Dict[str, Any]]:
...
if __name__ == "__main__":
    ...
    async def test_scraper():
        ...
        for paper in papers:
            ...
            print(
                f"Health AI Infrastructure Score: {paper['health_ai_infrastructure_score']}"
            )
