...
class AHAIIDataQualityManager:
    ...
    def __init__(self):
        ...
        self.org_patterns = {
            'world health organization': ['WHO', 'World Health Organization', 'world health org'],
            'african union': ['AU', 'African Union', 'african development bank'],
            'ministry of health': ['ministry of health', 'health ministry', 'dept of health', 'department of health'],
        ...
        self.health_ai_keywords = {
            'core_terms': [
                ...
                'clinical decision support', 'medical imaging ai', 'diagnostic ai'
            ],
            'health_domains': [
                'radiology', 'pathology', 'dermatology', 'cardiology', 'oncology',
                'telemedicine', 'electronic health records', 'clinical workflow',
                'population health', 'public health', 'healthcare analytics'
            ]
        }
    ...
    async def _validate_infrastructure_intelligence(self) -> List[ValidationIssue]:
        ...
        try:
            ...
            result = await supabase.table('infrastructure_intelligence').select('*').execute()
            records = result.data if result.data else []
            
            logger.info(f"Validating {len(records)} infrastructure intelligence records...")
            ...
            for record in records:
                ...
                if created_at:
                    ...
                    if datetime.now() - created_date > self.freshness_thresholds['infrastructure_intelligence']:
                        issues.append(ValidationIssue(
    ...
    async def _validate_infrastructure_indicators(self) -> List[ValidationIssue]:
        ...
        try:
            result = await supabase.table('infrastructure_indicators').select('*').execute()
            records = result.data if result.data else []
            
            logger.info(f"Validating {len(records)} infrastructure indicators...")
            
            # Valid pillar names
            valid_pillars = {'human_capital', 'physical_infrastructure', 'regulatory_framework', 'economic_market'}
            ...
            for record in records:
                ...
                if indicator_value is not None:
                    if indicator_unit == 'percentage' and (indicator_value < 0 or indicator_value > 100):
                        issues.append(ValidationIssue(
                            severity=ValidationSeverity.HIGH,
    ...
    async def _validate_health_ai_organizations(self) -> List[ValidationIssue]:
        ...
        try:
            result = await supabase.table('health_ai_organizations').select('*').execute()
            records = result.data if result.data else []
    ...
    async def _validate_ahaii_scores(self) -> List[ValidationIssue]:
        ...
        try:
            ...
            for record in records:
                ...
                score_fields = [
                    'total_score', 'human_capital_score', 'physical_infrastructure_score',
                    'regulatory_infrastructure_score', 'economic_market_score'
                ]
    ...
    async def _generate_quality_report(self, all_issues: Dict[str, List[ValidationIssue]]) -> Dict[str, Any]:
        ...
        report = {
            ...
            'recommendations': self._generate_quality_recommendations(all_issues, quality_score)
        }
        ...
        logger.info(f"   Medium: {severity_counts['medium']}, Low: {severity_counts['low']}")
        
        return report
    
    def _generate_quality_recommendations(self, all_issues: Dict[str, List[ValidationIssue]], 
                                        quality_score: float) -> List[str]:
        """Generate actionable quality improvement recommendations"""
        recommendations = []
        
        # Critical issues
        critical_count = sum(1 for issues in all_issues.values() for issue in issues 
                           if issue.severity == ValidationSeverity.CRITICAL)
        if critical_count > 0:
            recommendations.append(f"🚨 URGENT: Fix {critical_count} critical data quality issues immediately")
        
        # Missing country associations
        missing_countries = sum(1 for issues in all_issues.values() for issue in issues 
                              if issue.category == 'missing_country_association')
        if missing_countries > 0:
            recommendations.append(f"🌍 Improve country detection: {missing_countries} records need country associations")
        
        # URL validation issues
        url_issues = sum(1 for issues in all_issues.values() for issue in issues 
        ...
        # Data freshness
        freshness_issues = sum(1 for issues in all_issues.values() for issue in issues 
        ...
        if quality_score < 70:
            recommendations.append("📈 Overall data quality is below acceptable threshold - implement data validation pipeline")
        elif quality_score < 85:
            recommendations.append("🔧 Good data quality but room for improvement - focus on high-severity issues")
        else:
            recommendations.append("✅ Excellent data quality - maintain current standards")
        
        return recommendations
    
    async def auto_fix_issues(self, issues: List[ValidationIssue], 
        ...
        for issue in issues:
            try:
                if issue.category == 'missing_country_association' and issue.suggested_value:
                    # Auto-fix missing country associations where we have suggestions
                    if not dry_run:
                        # Look up country ID
                        country_result = await supabase.table('countries').select('id').eq('name', issue.suggested_value).execute()
                        if country_result.data:
                            country_id = country_result.data[0]['id']
                            # Update the record
                            await supabase.table(issue.table_name).update({'country_id': country_id}).eq('id', issue.record_id).execute()
                            fix_counts['fixed'] += 1
                        else:
                            fix_counts['failed'] += 1
                    else:
                    ...
                    if not dry_run:
                        await supabase.table(issue.table_name).update({'pillar': issue.suggested_value}).eq('id', issue.record_id).execute()
                        fix_counts['fixed'] += 1
                    else:
                        fix_counts['fixed'] += 1  # Would fix in real run
        ...
        logger.info(f"Auto-fix results: {fix_counts['fixed']} fixed, {fix_counts['skipped']} skipped, {fix_counts['failed']} failed")
        return fix_counts
...
if __name__ == "__main__":
    ...
    print(f"Total Issues Found: {sum(len(table_issues) for table_issues in issues.values())}")
