...
class HealthAICitation:
    ...
    infrastructure_pillar: Optional[str] = None  # human_capital, physical, regulatory, economic
...
class HealthAISnowballSampler:
    ...
    def __init__(self, config: SamplingConfig = None):
        ...
        self.session_id = f"health_ai_snowball_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        # Health AI patterns for extraction
        self.health_ai_patterns = {
            'health_terms': [
                'health', 'healthcare', 'medical', 'clinical', 'hospital', 'patient',
                'diagnosis', 'treatment', 'therapy', 'pharmaceutical', 'biomedical',
                'epidemiology', 'public health', 'telemedicine', 'telehealth'
            ],
            'ai_terms': [
                'artificial intelligence', 'machine learning', 'deep learning', 'ai',
                'ml', 'neural network', 'computer vision', 'nlp', 'natural language',
                'predictive analytics', 'algorithm', 'automated', 'intelligent'
            ],
            'infrastructure_terms': [
                'infrastructure', 'system', 'platform', 'implementation', 'deployment',
                'capacity', 'framework', 'architecture', 'ecosystem', 'network'
            ]
        }
        
        # African countries for relevance scoring
        self.african_countries = {
            'nigeria', 'kenya', 'south africa', 'ghana', 'egypt', 'morocco', 'rwanda',
            'uganda', 'tanzania', 'ethiopia', 'senegal', 'tunisia', 'algeria', 'cameroon',
            'ivory coast', 'zambia', 'zimbabwe', 'botswana', 'namibia', 'malawi',
            'mozambique', 'madagascar', 'angola', 'sudan', 'mali', 'burkina faso',
            'niger', 'chad', 'libya', 'mauritania', 'somalia', 'eritrea', 'djibouti',
            'gambia', 'guinea', 'sierra leone', 'liberia', 'togo', 'benin', 'gabon',
            'equatorial guinea', 'central african republic', 'democratic republic congo',
            'republic of congo', 'lesotho', 'swaziland', 'comoros', 'seychelles',
            'mauritius', 'cape verde', 'sao tome and principe'
        }
    ...
    async def run_sampling_session(self) -> Dict[str, Any]:
        ...
        session_stats = {
            ...
            "quality_scores": [],
            "pillar_distribution": {"human_capital": 0, "physical_infrastructure": 0, "regulatory": 0, "economic": 0}
        }
        ...
        try:
            ...
            while current_queue and current_depth < self.config.max_depth:
                logger.info(f"🔍 Processing depth {current_depth} with {len(current_queue)} items")
                ...
                session_stats["health_ai_discoveries"] += depth_results["discoveries_count"]
                session_stats["reference_links_extracted"] += depth_results["references_extracted"]
                session_stats["government_docs_processed"] += depth_results["government_docs_count"]
                session_stats["african_relevant_findings"] += depth_results["african_relevant_count"]
            ...
            session_stats["average_quality"] = (
                sum(session_stats["quality_scores"]) / len(session_stats["quality_scores"])
                if session_stats["quality_scores"] else 0
            )
            
            # Store session results
            await self._store_session_results(session_stats)
            
            logger.info(f"🎉 Snowball sampling completed: {session_stats['health_ai_discoveries']} new health AI discoveries")
    ...
    async def _create_health_ai_discovery_queue(self) -> List[Dict[str, Any]]:
        ...
        intelligence_result = await supabase.table("infrastructure_intelligence")\
            .select("*")\
        ...
        logger.info(f"📋 Created health AI discovery queue with {len(discovery_queue)} high-quality items")
    ...
    async def _process_depth_level(self, queue: List[Dict[str, Any]], depth: int) -> Dict[str, Any]:
        ...
        results = {
            ...
            "quality_scores": [],
            "next_depth_queue": [],
            "pillar_counts": {"human_capital": 0, "physical_infrastructure": 0, "regulatory": 0, "economic": 0}
        }
        ...
        for citation_item in queue:
            try:
                ...
                if citation_results["success"]:
                    ...
                    results["references_extracted"] += citation_results["references_extracted"]
                    results["government_docs_count"] += citation_results["government_docs_count"]
                    results["african_relevant_count"] += citation_results["african_relevant_count"]
    ...
    async def _process_single_citation(self, citation_item: Dict[str, Any], depth: int) -> Dict[str, Any]:
        ...
        if self._is_government_domain(citation_url):
            if not self._should_process_government_doc(citation_url):
                logger.info(f"⚠️ Skipping government document (policy restrictions): {citation_url}")
                return result
            else:
                result["government_docs_count"] = 1
                logger.info(f"🏛️ Processing approved government document: {citation_url}")
        ...
        try:
            ...
            if content:
                ...
                health_ai_citation = await self._extract_health_ai_info(content, citation_item, depth)
                
                if health_ai_citation and health_ai_citation.confidence_score >= self.config.min_confidence_threshold:
                    # Extract reference links (the key feature!)
                    reference_links = await self._extract_reference_links(content, citation_url)
                    ...
                    stored_citation = await self._store_health_ai_citation(health_ai_citation)
                    ...
                    if stored_citation:
                        ...
                        if health_ai_citation.infrastructure_pillar:
                            result["pillars_found"].append(health_ai_citation.infrastructure_pillar)
                        ...
                        for ref_link in reference_links:
                            ref_citation = {
                                ...
                                "url": ref_link,
                                "priority_score": health_ai_citation.confidence_score * 0.8,  # Slightly lower
                        ...
                        logger.info(f"✅ Processed health AI citation: {health_ai_citation.title[:50]}... ({len(reference_links)} refs)")
    ...
    async def _extract_health_ai_info(self, content: str, citation_item: Dict[str, Any], depth: int) -> Optional[HealthAICitation]:
        ...
        confidence = (health_score * 0.3 + ai_score * 0.3 + african_score * 0.2 + infrastructure_score * 0.2)
    ...
    async def _extract_reference_links(self, content: str, base_url: str) -> List[str]:
        ...
        reference_patterns = [
            'references', 'bibliography', 'citations', 'sources', 'further reading',
            'related work', 'see also', 'additional resources'
        ]
        ...
        reference_lists = soup.find_all(['ol', 'ul'], class_=re.compile(r'ref|citation|bibliography', re.I))
        ...
        for link in reference_links:
            ...
            absolute_link = urljoin(base_url, link) if not link.startswith('http') else link
    ...
    def _is_academic_reference(self, href: str) -> bool:
        ...
        academic_domains = [
            'doi.org', 'pubmed', 'arxiv.org', 'scholar.google', 'researchgate.net',
            'academia.edu', 'jstor.org', 'springer.com', 'wiley.com', 'elsevier.com',
            'nature.com', 'science.org', 'plos.org', 'bmj.com', 'nejm.org',
            'thelancet.com', 'cell.com', 'frontiersin.org', 'mdpi.com'
        ]
    ...
    def _determine_infrastructure_pillar(self, text: str) -> Optional[str]:
        ...
        pillar_keywords = {
            'human_capital': [
                'training', 'education', 'skills', 'workforce', 'capacity building',
                'curriculum', 'certification', 'competency', 'expertise', 'talent'
            ],
            'physical_infrastructure': [
                'hardware', 'server', 'network', 'connectivity', 'bandwidth',
                'data center', 'cloud', 'system', 'platform', 'infrastructure'
            ],
            'regulatory': [
                'regulation', 'policy', 'governance', 'compliance', 'standard',
                'law', 'legal', 'approval', 'certification', 'framework'
            ],
            'economic': [
                'funding', 'investment', 'economic', 'financial', 'cost',
                'budget', 'resource', 'market', 'commercial', 'revenue'
            ]
        }
    ...
    async def _store_health_ai_citation(self, citation: HealthAICitation) -> Optional[str]:
        ...
        try:
            ...
            result = await self.db_service.insert_infrastructure_intelligence(intelligence_data)
    ...
    def _should_include_in_next_depth(self, citation: Dict[str, Any]) -> bool:
        ...
        min_threshold = self.config.min_confidence_threshold + (0.1 * len(self.processed_urls) / 100)
        
        return (
            priority_score >= min_threshold and
            citation_type in ["academic_paper", "policy_document", "health_ai_report"] and
            citation.get("url") not in self.processed_urls
        )
...
async def run_health_ai_snowball_sampling(config: SamplingConfig = None) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    async def test_snowball_sampling():
        ...
        config = SamplingConfig(
            ...
            government_domains_allowed={'who.int', 'afro.who.int'}  # Only international orgs
        )
