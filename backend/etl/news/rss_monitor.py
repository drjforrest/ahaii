"""
RSS Article Monitor for AHAII
Health AI Infrastructure news monitoring pipeline for African health tech intelligence
"""
...
from .health_ai_infrastructure_signal_processor import HealthAIInfrastructureSignalProcessor
...
class NewsArticle(BaseModel):
    ...
    # Health AI Infrastructure Analysis Results
    infrastructure_indicators: List[Dict[str, Any]] = []
    health_ai_relevance_score: float = 0.0
    african_relevance_score: float = 0.0
    infrastructure_pillar: Optional[str] = None
    mentioned_health_organizations: List[str] = []
...
class RSSMonitor:
    """RSS feed monitor for African health AI infrastructure news"""
    
    def __init__(self):
        self.rss_feeds = self.get_health_ai_infrastructure_feeds()
        self.session = None
        self.african_countries = set(settings.AFRICAN_COUNTRIES)
        self.signal_processor = HealthAIInfrastructureSignalProcessor()
        self.infrastructure_pillars = {
            'human_capital', 'physical_infrastructure', 'regulatory', 'economic'
        }
    
    def get_health_ai_infrastructure_feeds(self) -> List[str]:
        """Get health AI infrastructure specific RSS feeds"""
        return [
            ...
            'https://techcabal.com/category/health/feed/',
            'https://disrupt-africa.com/category/healthtech/feed/',
            'https://ventureburn.com/category/health/feed/',
            'https://www.itnewsafrica.com/category/healthcare/feed/',
            'https://www.bizcommunity.com/RSS/196/149/1.html'  # Healthcare IT in Africa
        ]
    ...
    async def __aenter__(self):
        self.session = aiohttp.ClientSession(
            ...
            headers={
            'User-Agent': 'AHAII Health AI Infrastructure Monitor/1.0 (Research Bot)'
            }
        )
        return self
    ...
    def extract_source_name(self, feed_url: str) -> str:
        ...
        # Health AI Infrastructure specific sources
        if 'healthcareitnews' in domain:
            return 'Healthcare IT News'
        elif 'mobihealthnews' in domain:
            return 'MobiHealthNews'
        elif 'africanews.com' in domain:
            return 'Africanews Health'
        elif 'who.int' in domain:
            return 'World Health Organization'
        elif 'himss.org' in domain:
            return 'HIMSS'
        elif 'medcitynews' in domain:
            return 'MedCity News'
        elif 'healthdatamanagement' in domain:
            return 'Health Data Management'
        elif 'modernhealthcare' in domain:
            return 'Modern Healthcare'
        elif 'techcabal' in domain:
            return 'TechCabal Health'
        elif 'ventureburn' in domain:
            return 'Ventureburn Health'
        elif 'disrupt-africa' in domain:
            return 'Disrupt Africa HealthTech'
        elif 'itnewsafrica' in domain:
            return 'IT News Africa Healthcare'
        elif 'bizcommunity' in domain:
            return 'Bizcommunity Healthcare'
        else:
        ...
        for feed_url in self.rss_feeds:
            try:
                ...
                for article_data in articles_data:
                    ...
                    # Analyze relevance and health AI infrastructure signals
                    analysis = self.analyze_article_relevance(article_data)
                    infrastructure_analysis = self.signal_processor.classify_infrastructure_signal(article_data)
                    
                    # Combine analyses
                    article_data.update(analysis)
                    article_data.update({
                        'health_ai_relevance_score': max(analysis['ai_relevance_score'], max(infrastructure_analysis['pillar_scores'].values()) if infrastructure_analysis['pillar_scores'] else 0),
                        'infrastructure_pillar': self.signal_processor.get_primary_pillar(infrastructure_analysis),
                        'infrastructure_indicators': infrastructure_analysis['infrastructure_indicators'],
                        'mentioned_health_organizations': infrastructure_analysis['health_organizations'],
                        'regulatory_signals': infrastructure_analysis['regulatory_signals']
                    })
                    
                    # Filter by health AI infrastructure relevance
                    is_relevant = (
                        analysis['ai_relevance_score'] >= 0.3 or 
                        infrastructure_analysis['significance'] in ['high', 'medium'] or
                        infrastructure_analysis['confidence_score'] > 0.4
                    )
                    
                    is_african_relevant = (
                        analysis['african_relevance_score'] >= 0.2 or
                        any(country in analysis['mentioned_countries'] for country in self.african_countries)
                    )
                    
                    if is_relevant and is_african_relevant:
...
if __name__ == "__main__":
    # Test the RSS monitor with health AI infrastructure focus
    async def test_monitor():
        articles = await monitor_rss_feeds(hours_back=48)
        
        print(f"\n🔍 Health AI Infrastructure News Monitor Results")
        ...
        for i, article in enumerate(articles[:5], 1):  # Show first 5
            ...
            print(f"Health AI Relevance: {article.health_ai_relevance_score:.2f}")
            print(f"African Relevance: {article.african_relevance_score:.2f}")
            print(f"Infrastructure Pillar: {article.infrastructure_pillar or 'None'}")
            print(f"Health Organizations: {article.mentioned_health_organizations}")
            print(f"Countries: {article.mentioned_countries}")
            print(f"Infrastructure Indicators: {len(article.infrastructure_indicators)}")
            print(f"Regulatory Signals: {len(article.regulatory_signals)}")
            print(f"Funding Mentions: {len(article.funding_mentions)}")
            if article.funding_mentions:
                for funding in article.funding_mentions[:2]:
                    print(f"  - {funding['formatted_amount']}")
