...
class HealthAIInfrastructureSignalProcessor:
    ...
    def __init__(self):
        self.infrastructure_keywords = {
            'human_capital': [
                'medical training', 'clinical education', 'health informatics curriculum',
                'ai medical training', 'clinical ai literacy', 'medical ai workforce',
                'health data science programs', 'clinical decision support training',
                'biomedical informatics education', 'health ai capacity building',
                'medical informatics certification', 'clinical ai competency',
                'health ai skills development', 'medical ai education program'
            ],
            'physical_infrastructure': [
                'hospital emr implementation', 'medical imaging systems deployment',
                'healthcare data center', 'clinical data network', 'telemedicine platform',
                'pacs implementation', 'health information system', 'hospital it infrastructure',
                'clinical decision support system', 'electronic health record',
                'medical device connectivity', 'healthcare cloud infrastructure',
                'hospital digital transformation', 'clinical data integration platform',
                'health information exchange', 'medical iot deployment'
            ],
            'regulatory': [
                'medical ai regulation', 'health ai governance framework',
                'clinical ai validation', 'medical device ai approval', 'fda ai guidance',
                'health data privacy regulation', 'clinical trial ai oversight',
                'medical ai safety standard', 'health ai ethics committee',
                'medical device regulation', 'clinical ai certification',
                'health ai compliance framework', 'medical ai audit requirement',
                'clinical ai quality assurance', 'health ai regulatory pathway'
            ],
            'economic': [
                'health ai funding', 'medical ai investment', 'healthtech venture capital',
                'digital health funding round', 'clinical ai market analysis',
                'health ai reimbursement', 'medical ai cost effectiveness',
                'healthtech ipo', 'digital health merger', 'clinical ai roi',
                'health ai market valuation', 'medical ai budget allocation',
                'healthtech acquisition', 'digital health economic impact'
            ]
        }
    ...
    def classify_infrastructure_signal(self, article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        classification['infrastructure_indicators'] = self._extract_infrastructure_indicators(full_text)
        
        # Extract health organizations
        classification['health_organizations'] = self._extract_health_organizations(full_text)
        
        # Extract regulatory signals
        classification['regulatory_signals'] = self._extract_regulatory_signals(full_text)
        
        # Extract funding mentions
        classification['funding_mentions'] = self._extract_funding_mentions(full_text)
        
        # Calculate overall significance
        active_pillars = sum(classification[pillar] for pillar in ['human_capital', 'physical_infrastructure', 'regulatory', 'economic'])
    ...
    def _calculate_pillar_score(self, text: str, keywords: List[str]) -> float:
        ...
        for keyword in keywords:
            ...
            phrase_count = len(re.findall(rf'\b{re.escape(keyword)}\b', text, re.IGNORECASE))
            if phrase_count > 0:
    ...
    def _calculate_confidence_score(self, pillar_scores: Dict[str, float], 
                                  indicators: List[Dict], 
                                  regulatory_signals: List[Dict],
                                  funding_mentions: List[Dict]) -> float:
        """Calculate overall confidence score for the classification"""
        
        # Base score from pillar analysis
        max_pillar_score = max(pillar_scores.values()) if pillar_scores else 0
        avg_pillar_score = sum(pillar_scores.values()) / len(pillar_scores) if pillar_scores else 0
    ...
    def is_significant_infrastructure_signal(self, classification: Dict[str, Any]) -> bool:
...
if __name__ == "__main__":
    ...
    print(f"Active Pillars: {[k for k, v in classification.items() if k in ['human_capital', 'physical_infrastructure', 'regulatory', 'economic'] and v]}")
    print(f"Infrastructure Indicators: {len(classification['infrastructure_indicators'])}")
