...
from services.advanced_ai_deduplication_service import (
    analyze_articles_with_complex_relationships,
)
...
class AfricanAINewsScraper:
    ...
    async def scrape_african_ai_news(
        ...
        async with self.serper as serper:
            for i, query in enumerate(news_queries):
                try:
                    logger.info(
                        f"📰 Searching news query {i+1}/{len(news_queries)}: {query[:50]}..."
                    )

                    # Search news with date filter
                    search_response = await serper.search_news(
                        query=query, num_results=results_per_query, days_back=days_back
                    )
        ...
        sorted_articles = sorted(
            scored_articles,
            key=lambda a: a.african_relevance_score
            + a.ai_relevance_score
            + a.innovation_relevance_score,
            reverse=True,
        )
        ...
        if len(sorted_articles) > 1:
            logger.info(
                "🧠 Applying advanced AI relationship analysis and deduplication..."
            )
            ...
            canonical_articles, relationship_metadata = (
                await analyze_articles_with_complex_relationships(article_dicts)
            )
                ...
                for orig_article in sorted_articles:
                    if str(orig_article.url) == article_dict["url"]:
                        ...
                        if "cluster_info" in article_dict:
                            ...
                            orig_article.cluster_size = cluster_info.get(
                                "cluster_size", 1
                            )
                            orig_article.cluster_type = cluster_info.get("cluster_type")
                            orig_article.relationship_summary = cluster_info.get(
                                "relationship_summary"
                            )
                            orig_article.related_articles = cluster_info.get(
                                "related_articles", 0
                            )
                        else:
            ...
            logger.info(
                f"🧠 Advanced AI analysis: {len(sorted_articles)} → {len(final_articles)} unique events"
            )
            logger.info(
                f"🔗 Relationships found: {clusters_found} clusters, {standalone_events} standalone, {total_relationships} total relationships"
            )

            # Log cluster details
            for cluster in relationship_metadata.get("clusters", []):
                logger.info(
                    f"📦 {cluster['type'].title()}: {cluster['primary_entity']} ({cluster['event_count']} events) - {cluster['summary']}"
                )
        ...
        for result in search_results:
            try:
                ...
                article = NewsArticle(
                    ...
                    or self._extract_source_from_url(str(result.link)),
    ...
    def _score_and_filter_articles(
        ...
        for article in articles:
            ...
            article.innovation_relevance_score = self._calculate_innovation_relevance(
                article
            )
            ...
            if (
                article.african_relevance_score >= 0.3
                and article.ai_relevance_score >= 0.2
                and article.innovation_relevance_score >= 0.2
            ):
                scored_articles.append(article)

        logger.info(
            f"🎯 Filtered {len(articles)} -> {len(scored_articles)} relevant articles"
        )
...
async def scrape_african_ai_news(
    ...
    async with AfricanAINewsScraper() as scraper:
        ...
        logger.info(
            f"🎯 Advanced African AI news scraping complete: {len(articles)} unique events found"
        )
        ...
        for article in articles:
            if hasattr(article, "cluster_info"):
                ...
                logger.info(
                    f"📰 {article.title[:50]}... - {cluster_info['cluster_type']} cluster (size: {cluster_info['cluster_size']})"
                )
            else:
...
if __name__ == "__main__":
    ...
    async def test_scraper():
        articles = await scrape_african_ai_news(days_back=14, max_results=20)
        ...
        for i, article in enumerate(articles[:10], 1):
            ...
            print(
                f"   Scores - African: {article.african_relevance_score:.2f} | AI: {article.ai_relevance_score:.2f} | Innovation: {article.innovation_relevance_score:.2f}"
            )
            ...
            if hasattr(article, "cluster_info"):
                cluster_info = article.cluster_info
                print(
                    f"   🔗 Cluster: {cluster_info['cluster_type']} (size: {cluster_info['cluster_size']}) - {cluster_info['relationship_summary']}"
                )
                print(
                    f"   📈 Impact: {len(cluster_info.get('total_impact', {}).get('companies_involved', []))} companies involved"
                )
            else:
