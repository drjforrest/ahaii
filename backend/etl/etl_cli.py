...
@click.option(
    "--component",
    "-c",
    type=click.Choice(["news", "academic", "scoring", "snowball", "quality", "all"]),
    default="all",
    ...
    async def _run_pipeline():
        ...
        try:
            ...
            if isinstance(result, dict) and all(
                hasattr(r, "status") for r in result.values()
            ):
                ...
                for name, run in result.items():
                    ...
                    click.echo(
                        f"   {status_icon} {name}: {run.records_created} records created, {run.records_failed} failed"
                    )
            else:
    ...
    async def _show_status():
        ...
        stats_table = [
            ...
            [
                ...
                stats.get("total_infrastructure_indicators", 0),
            ],
            ["Health AI Organizations", stats.get("total_health_ai_organizations", 0)],
            [
                "Infrastructure Intelligence",
                stats.get("total_infrastructure_intelligence", 0),
            ],
...
@pipeline.command()
@click.option(
    "--max-depth", type=int, default=2, help="Maximum snowball sampling depth"
)
@click.option(
    "--max-citations", type=int, default=10, help="Maximum citations per batch"
)
@click.option(
    "--government-docs",
    is_flag=True,
    help="Enable government document processing (use with caution)",
)
def snowball(max_depth, max_citations, government_docs):
    ...
    click.echo(
        f"🔬 Starting Snowball Sampling (depth: {max_depth}, citations: {max_citations})..."
    )

    if government_docs:
        click.echo(
            "⚠️  Government document processing enabled - ensuring safe operation..."
        )
    ...
    async def _run_snowball():
        ...
        config = SamplingConfig(
            ...
            government_domains_allowed=(
                {"who.int", "afro.who.int", "africa.who.int"}
                if government_docs
                else set()
            ),
        )
        ...
        click.echo(
            f"   🔗 Citations processed: {results.get('citations_processed', 0)}"
        )
        click.echo(
            f"   🧬 Health AI discoveries: {results.get('health_ai_discoveries', 0)}"
        )
        click.echo(
            f"   🌍 African relevant findings: {results.get('african_relevant_findings', 0)}"
        )
        click.echo(
            f"   🏛️  Government docs processed: {results.get('government_docs_processed', 0)}"
        )
...
def fix(dry_run):
    ...
    async def _auto_fix():
        ...
        try:
            ...
            # Attempt auto-fix
            fix_results = await manager.quality_manager.auto_fix_issues(
                all_issues_list, dry_run=dry_run
            )
...
@monitor.command()
@click.option(
    "--watch", "-w", is_flag=True, help="Watch mode - refresh every 30 seconds"
)
def dashboard(watch):
        ...
        try:
            ...
            main_stats = [
                ...
                [
                    ...
                    stats.get("total_health_ai_organizations", 0),
                ],
                [
                    "Infrastructure Intelligence",
                    stats.get("total_infrastructure_intelligence", 0),
            ...
            click.echo(
                tabulate(main_stats, headers=["Table", "Records"], tablefmt="grid")
            )
...
def config(format):
    ...
    config_data = {
        "version": "1.0.0",
        "components": {
            "news_monitoring": {"enabled": True, "frequency": "6 hours", "sources": 13},
            "academic_processing": {
                "enabled": True,
                "frequency": "daily",
