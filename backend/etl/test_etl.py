...
class ETLTestSuite:
    ...
    async def test_news_monitoring(self) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            details = {
                ...
                "sources_active": (
                    len(set(article.source for article in articles)) if articles else 0
                ),
                ...
                "pillars_detected": list(
                    set(
                        article.infrastructure_pillar
                        for article in articles
                        if article.infrastructure_pillar
                    )
                ),
        ...
        try:
            ...
            details = {
                ...
                "news_pipeline_status": (
                    ...
                    if hasattr(news_result, "status")
                    else "unknown"
                ),
                "data_changes": {
                    "intelligence_records": final_stats.get(
                        "total_infrastructure_intelligence", 0
                    )
                    - initial_stats.get("total_infrastructure_intelligence", 0),
                    "indicators": final_stats.get("total_infrastructure_indicators", 0)
                    - initial_stats.get("total_infrastructure_indicators", 0),
                },
            }
    ...
    def print_test_summary(self, results: Dict[str, Any]):
        ...
        print(
            f"📊 Tests: {results['summary']['total']} total, {results['summary']['passed']} passed, {results['summary']['failed']} failed"
        )
        ...
        for test in results["tests"]:
            ...
            print(
                f"{status_icon} {test['name']}: {test['status']} ({test['duration_seconds']:.1f}s)"
            )

            if test["status"] == "FAILED":
                print(
                    f"   Error: {test.get('error', test.get('message', 'Unknown error'))}"
                )
            else:
