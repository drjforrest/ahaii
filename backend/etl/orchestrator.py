...
from .academic.unified_academic_processor import UnifiedAcademicProcessor, HealthAIInfrastructureExtractor
...
class AHAIIETLOrchestrator:
    ...
    def __init__(self):
        ...
        snowball_config = SamplingConfig(
            ...
            government_domains_allowed={'who.int', 'afro.who.int', 'africa.who.int'}  # International only
        )
    ...
    async def run_full_pipeline(self) -> Dict[str, PipelineRun]:
        ...
        if self.pipeline_configs["academic_processing"]["enabled"]:
            results["academic_processing"] = await self.run_academic_processing_pipeline()
    ...
    async def run_news_monitoring_pipeline(self) -> PipelineRun:
        ...
        try:
            ...
            for article in articles:
                try:
                    ...
                    intelligence_data = {
                        ...
                        "report_summary": article.summary or article.content[:500] if article.content else "",
                        "key_findings": {
                        ...
                        "affects_human_capital": article.infrastructure_pillar == "human_capital",
                        "affects_physical_infrastructure": article.infrastructure_pillar == "physical_infrastructure", 
                        "affects_regulatory_framework": article.infrastructure_pillar == "regulatory",
                        "affects_economic_market": article.infrastructure_pillar == "economic",
                        "impact_significance": "high" if article.health_ai_relevance_score > 0.7 else "medium" if article.health_ai_relevance_score > 0.4 else "low",
                        "confidence_score": max(article.health_ai_relevance_score, article.african_relevance_score),
                    ...
                    if article.mentioned_countries:
                        for country_name in article.mentioned_countries:
                            # Look up country ID
                            country_result = await supabase.table("countries").select("id").ilike("name", f"%{country_name}%").execute()
                            if country_result.data:
                                intelligence_data["country_id"] = country_result.data[0]["id"]
                                break
                    
                    stored_record = await self.db_service.insert_infrastructure_intelligence(intelligence_data)
                    if stored_record:
                            ...
                            for country_name in article.mentioned_countries:
                                ...
                                if country_result.data:
                                    indicator_data = {
                                        ...
                                        "pillar": self._map_pillar_to_ahaii(article.infrastructure_pillar),
                                        "indicator_name": indicator.get("indicator_name", "news_signal"),
                                        "indicator_value": indicator.get("value", 1.0),
                                        "indicator_unit": indicator.get("value_type", "signal"),
                                        "data_year": article.published_date.year if article.published_date else datetime.now().year,
                                    ...
                                    await self.db_service.insert_infrastructure_indicator(indicator_data)
                                    break
            ...
            run.metadata = {
                ...
                "intelligence_records_created": len(infrastructure_intelligence_records),
                "sources_processed": len(set(article.source for article in articles)),
                "countries_mentioned": len(set(country for article in articles for country in article.mentioned_countries)),
            ...
            logger.info(f"✅ News Monitoring Pipeline Completed: {run.records_created} records created, {run.records_failed} failed")
        ...
        try:
            ...
            for paper in papers:
                try:
                    ...
                    indicators = self.infrastructure_extractor.extract_infrastructure_indicators(paper)
                    ...
                    for indicator in indicators:
                        ...
                        if paper.get("african_entities"):
                            for entity in paper["african_entities"]:
                                country_result = await supabase.table("countries").select("id").ilike("name", f"%{entity}%").execute()
                                if country_result.data:
                                    country_id = country_result.data[0]["id"]
                                    break
                        ...
                        if country_id:
                            ...
                            indicator["data_year"] = paper.get("year", datetime.now().year)
                            
                            stored_indicator = await self.db_service.insert_infrastructure_indicator(indicator)
                            if stored_indicator:
                                run.records_created += 1
                            else:
                                run.records_failed += 1
                    ...
                    intelligence_data = {
                        ...
                        "confidence_score": (paper.get("african_relevance_score", 0) + paper.get("ai_relevance_score", 0)) / 2,
                        "verification_status": "peer_reviewed" if paper.get("source") in ["pubmed", "systematic_review"] else "auto_processed"
                    }
            ...
            logger.info(f"✅ Academic Processing Pipeline Completed: {run.records_created} indicators created")
    ...
    async def run_scoring_pipeline(self) -> PipelineRun:
        ...
        try:
            ...
            countries_result = await supabase.table("countries").select("*").eq("continent", "Africa").execute()
            ...
            for country in countries:
                try:
                    ...
                    scores = await self.scoring_service.calculate_comprehensive_ahaii_score(
                    ...
                    if scores:
                        ...
                        stored_scores = await self.db_service.update_ahaii_scores(country["id"], scores)
                        if stored_scores:
                            run.records_created += 1
                        else:
                            run.records_failed += 1
                    ...
                    logger.error(f"Error calculating scores for {country.get('name', 'Unknown')}: {e}")
                    run.records_failed += 1
                    continue
            ...
            logger.info(f"✅ Scoring Pipeline Completed: {run.records_created} countries scored")
    ...
    async def run_snowball_sampling_pipeline(self) -> PipelineRun:
        ...
        try:
            ...
            run.metadata = {
                ...
                "reference_links_extracted": session_results.get("reference_links_extracted", 0),
                "government_docs_processed": session_results.get("government_docs_processed", 0),
                "african_relevant_findings": session_results.get("african_relevant_findings", 0),
            ...
            logger.info(f"✅ Snowball Sampling Pipeline Completed: {run.records_created} discoveries, {run.metadata['reference_links_extracted']} references")
    ...
    async def run_data_quality_pipeline(self) -> PipelineRun:
        ...
        try:
            ...
            duplicates_result = await supabase.rpc("check_duplicate_intelligence").execute()
            if duplicates_result.data:
                quality_checks["duplicate_intelligence_records"] = duplicates_result.data[0].get("count", 0)
            
            # Check for missing country associations
            missing_country_result = await supabase.table("infrastructure_intelligence").select("id").is_("country_id", "null").execute()
            if missing_country_result.data:
                quality_checks["missing_country_associations"] = len(missing_country_result.data)
            
            # Check for outdated indicators (older than 2 years)
            cutoff_year = datetime.now().year - 2
            outdated_result = await supabase.table("infrastructure_indicators").select("id").lt("data_year", cutoff_year).execute()
            if outdated_result.data:
                quality_checks["outdated_indicators"] = len(outdated_result.data)
            
            # Check for low confidence scores
            low_confidence_result = await supabase.table("infrastructure_indicators").select("id").lt("confidence_score", 0.3).execute()
            if low_confidence_result.data:
                quality_checks["low_confidence_scores"] = len(low_confidence_result.data)
            
            # Check for unverified records
            unverified_result = await supabase.table("infrastructure_intelligence").select("id").eq("verification_status", "pending").execute()
            ...
            run.metadata = {
                ...
                "overall_quality_score": max(0, 1.0 - (sum(quality_checks.values()) / 1000))  # Simple quality metric
            }
            
            run.status = PipelineStatus.COMPLETED
            run.completed_at = datetime.now()
            
            logger.info(f"✅ Data Quality Pipeline Completed: {run.records_processed} issues identified")
    ...
    async def generate_pipeline_report(self, results: Dict[str, PipelineRun]) -> Dict[str, Any]:
        ...
        for pipeline_name, run in results.items():
            ...
            report["overall_summary"]["total_records_processed"] += run.records_processed
        ...
        if report["overall_summary"]["failed_pipelines"] > 0:
            report["recommendations"].append("Review failed pipelines and implement error handling improvements")
        
        if report["overall_summary"]["total_records_failed"] > report["overall_summary"]["total_records_created"] * 0.1:
            report["recommendations"].append("High failure rate detected - review data validation logic")
        
        logger.info("📊 Pipeline Execution Report Generated")
        logger.info(f"   Successful Pipelines: {report['overall_summary']['successful_pipelines']}/{report['overall_summary']['total_pipelines']}")
        logger.info(f"   Records Created: {report['overall_summary']['total_records_created']}")
        logger.info(f"   Records Failed: {report['overall_summary']['total_records_failed']}")
...
if __name__ == "__main__":
    ...
    print(f"Results: {json.dumps({k: v.status.value for k, v in results.items()}, indent=2)}")
