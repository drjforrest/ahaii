...
class AHAIIETLScheduler:
    ...
    def add_scheduled_task(self, task: ScheduledTask) -> bool:
        ...
        try:
            ...
            if not self._validate_schedule_expression(task.schedule_type, task.schedule_expression):
                logger.error(f"Invalid schedule expression: {task.schedule_expression}")
                return False
            
            # Calculate next run time
            task.next_run = self._calculate_next_run(task.schedule_type, task.schedule_expression)
            
            self.scheduled_tasks[task.task_id] = task
            logger.info(f"📅 Added scheduled task: {task.name} ({task.schedule_expression})")
    ...
    async def _execute_task(self, task: ScheduledTask):
        ...
        try:
            ...
            logger.info(f"✅ Task completed: {task.name} ({duration.total_seconds():.1f}s)")
    ...
    async def _handle_task_failure(self, task: ScheduledTask, error_msg: str):
        ...
        if task.notify_on_failure:
            await self._send_notification(task, "failure", {"error": error_msg, "failure_count": task.failure_count})
        ...
        if task.failure_count >= task.max_failures:
            ...
            await self._send_notification(task, "critical_failure", {
                "error": error_msg,
    ...
    async def _send_notification(self, task: ScheduledTask, status: str, data: Any):
        ...
        try:
            ...
            logger.info(f"   Data: {json.dumps(notification_data, indent=2, default=str)}")
            
        except Exception as e:
            logger.error(f"Failed to send notification: {e}")
    
    def _validate_schedule_expression(self, schedule_type: ScheduleType, expression: str) -> bool:
    ...
    def _calculate_next_run(self, schedule_type: ScheduleType, expression: str) -> datetime:
