...
class CountryIntelligenceMatcher:
    ...
    def _build_country_patterns(self) -> Dict[str, List[str]]:
        ...
        patterns = {
            ...
            'ZAF': ['south africa', 'south african', 'johannesburg', 'cape town', 'durban', 'pretoria'],
...
class IndicatorExtractor:
    ...
    def extract_indicators(self, text: str, country_iso: str) -> List[InfrastructureSignal]:
            ...
            for pattern_config in pattern_configs:
                ...
                for match in matches:
                    try:
                        ...
                        logger.warning(f"Error extracting value from pattern match: {e}")
                        continue
...
class IntelligenceProcessingService:
    ...
    async def process_intelligence_report(self, report_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Detect country
        country_iso = self.country_matcher.detect_country(f"{report_title} {report_content}")
        ...
        logger.info(f"Processed intelligence for {country_iso}: {len(signals)} signals extracted")
    ...
    async def _store_infrastructure_signals(self, signals: List[InfrastructureSignal]):
        ...
        for signal in signals:
            try:
                # Get country ID
                country_result = self.supabase.table('countries').select('id').eq('iso_code_alpha3', signal.country_iso_code).execute()
                ...
                result = self.supabase.table('infrastructure_indicators').insert(indicator_data).execute()
                
                if result.data:
                    logger.info(f"Stored indicator {signal.indicator_name} for {signal.country_iso_code}")
                else:
                ...
                intelligence_data = {
                    ...
                    'affects_physical_infrastructure': signal.pillar == 'physical_infrastructure',
                    'affects_regulatory_framework': signal.pillar == 'regulatory_infrastructure',
                    'affects_economic_market': signal.pillar == 'economic_market',
                    'impact_significance': 'medium' if signal.confidence_score > 0.7 else 'low',
                    'confidence_score': signal.confidence_score,
                    'processed_by_ai': True
                }
                
                self.supabase.table('infrastructure_intelligence').insert(intelligence_data).execute()
    ...
    async def _update_country_scores(self, country_iso: str) -> Optional[Dict]:
        ...
        try:
            # Get country ID
            country_result = self.supabase.table('countries').select('id, name').eq('iso_code_alpha3', country_iso).execute()
            ...
            indicators_result = self.supabase.table('infrastructure_indicators').select('*').eq('country_id', country_id).execute()
            ...
            for indicator in indicators_result.data:
                ...
                if indicator_name not in indicators_data:
                    ...
                    current_date = datetime.fromisoformat(indicator['created_at'].replace('Z', '+00:00'))
            ...
            scores_data = {
                ...
                'physical_infrastructure_score': new_scores['physical_infrastructure_score'],
                'regulatory_infrastructure_score': new_scores['regulatory_infrastructure_score'],
                ...
                'data_completeness_percentage': new_scores['data_completeness_percentage'],
                'key_strengths': new_scores['key_strengths'],
                'priority_improvement_areas': new_scores['priority_improvement_areas'],
                'assessment_methodology_version': new_scores['assessment_methodology_version'],
                'indicator_weights_used': new_scores['indicator_weights_used']
            }
            
            # Upsert scores (insert or update)
            upsert_result = self.supabase.table('ahaii_scores').upsert(
                scores_data, 
                on_conflict='country_id,assessment_year'
            ).execute()
            
            if upsert_result.data:
                logger.info(f"Updated AHAII scores for {country_name}: {new_scores['total_score']:.1f} (Tier {new_scores['readiness_tier']})")
                return {
                    ...
                    'updated_pillars': {
                        ...
                        'physical_infrastructure': new_scores['physical_infrastructure_score'],
                        'regulatory_infrastructure': new_scores['regulatory_infrastructure_score'],
