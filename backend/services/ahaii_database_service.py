...
from models.ahaii_models import (
    ...
    AHAIIAssessmentRequest,
    AHAIIAssessmentResponse,
    AHAIICountryScoreResponse,
)
...
class AHAIIDatabaseService:
    ...
    def create_assessment(
        self, countries: List[str], methodology_version: str = "1.0"
    ) -> str:
        """
        ...
        try:
            ...
            result = (
                self.supabase.table("ahaii_assessments")
            ...
            if result.data:
                logger.info(
                    f"Created assessment {assessment_id} for {len(countries)} countries"
                )
    ...
    def get_latest_assessment(self) -> Optional[Dict]:
        ...
        try:
            result = (
                self.supabase.table("ahaii_assessments")
                .select("*")
                .eq("data_collection_status", "completed")
    ...
    def save_country_score(self, assessment_id: str, country_score_data: Dict) -> int:
        ...
        try:
            ...
            result = (
                self.supabase.table("ahaii_country_scores").insert(score_data).execute()
            )

            if result.data:
                country_score_id = result.data[0]["id"]
                logger.info(
                    f"Saved country score for {country_score_data.get('country_name')}"
                )
    ...
    def get_country_scores(self, assessment_id: str) -> List[Dict]:
        ...
        try:
            result = (
                self.supabase.table("ahaii_country_scores")
                .select("*")
                .eq("assessment_id", assessment_id)
    ...
    def get_country_score_history(
        self, country_code: str, limit: int = 10
    ) -> List[Dict]:
        """Get score history for a specific country"""
        try:
            result = (
                self.supabase.table("ahaii_country_scores")
                .select("*, ahaii_assessments(assessment_date, methodology_version)")
    ...
    def save_pillar_scores(
        self, country_score_id: int, pillar_scores: List[Dict]
    ) -> bool:
        ...
        try:
            ...
            for pillar_score in pillar_scores:
                pillar_data.append(
                    {"country_score_id": country_score_id, **pillar_score}
                )

            result = (
                self.supabase.table("ahaii_pillar_scores").insert(pillar_data).execute()
            )
    ...
    def get_pillar_scores(self, country_score_id: int) -> List[Dict]:
        ...
        try:
            result = (
                self.supabase.table("ahaii_pillar_scores")
                .select("*")
                .eq("country_score_id", country_score_id)
                .execute()
            )
    ...
    def save_indicator_values(
        self, country_score_id: int, indicators: List[Dict]
    ) -> bool:
        ...
        try:
            ...
            for indicator in indicators:
                indicator_data.append(
                    {"country_score_id": country_score_id, **indicator}
                )

            result = (
                self.supabase.table("ahaii_indicator_values")
    ...
    def get_indicator_values(self, country_score_id: int) -> List[Dict]:
        ...
        try:
            result = (
                ...
                .select("*")
                .eq("country_score_id", country_score_id)
                .order("pillar_name", "indicator_name")
                .execute()
            )
    ...
    def get_indicators_by_pillar(
        self, assessment_id: str, pillar_name: str
    ) -> List[Dict]:
        """Get all indicators for a specific pillar across countries"""
        try:
            result = (
                self.supabase.table("ahaii_indicator_values")
                .select("*, ahaii_country_scores(country_code, country_name)")
    ...
    def create_data_collection_run(self, assessment_id: str) -> str:
        ...
        try:
            ...
            result = (
                self.supabase.table("data_collection_runs").insert(run_data).execute()
            )
    ...
    def update_collection_run_status(self, run_id: str, **updates) -> bool:
        ...
        try:
            ...
            result = (
                self.supabase.table("data_collection_runs")
                .update(updates)
                .eq("run_id", run_id)
                .execute()
            )
    ...
    def save_validation_result(self, validation_data: Dict) -> bool:
        ...
        try:
            result = (
                self.supabase.table("expert_validation_results")
    ...
    def get_regional_rankings(self, assessment_id: str) -> List[Dict]:
        ...
        try:
            result = (
                self.supabase.table("ahaii_country_rankings")
                .select("*")
                .eq("assessment_id", assessment_id)
                .execute()
            )
    ...
    def get_pillar_performance_summary(self, assessment_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = (
                self.supabase.table("ahaii_pillar_summary")
                .select("*")
                .eq("assessment_id", assessment_id)
                .execute()
            )
    ...
    def get_data_quality_summary(self, assessment_id: str) -> Dict[str, Any]:
        ...
        try:
            result = (
                self.supabase.table("ahaii_data_quality_summary")
                .select("*")
                .eq("assessment_id", assessment_id)
                .execute()
            )
            ...
            validated_indicators = sum(
                row["validated_indicators"] or 0 for row in result.data
            )
            ...
            return {
                ...
                "validation_rate": (
                    validated_indicators / total_indicators
                    if total_indicators > 0
                    else 0
                ),
                "proxy_rate": (
                    proxy_indicators / total_indicators if total_indicators > 0 else 0
                ),
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = (
                self.supabase.table("ahaii_assessments").select("id").limit(1).execute()
            )

            # Count records in key tables
            assessments_count = len(
                self.supabase.table("ahaii_assessments").select("id").execute().data
                or []
            )
            scores_count = len(
                self.supabase.table("ahaii_country_scores").select("id").execute().data
                or []
            )
    ...
    def save_complete_assessment_data(
        self, assessment_id: str, assessment_data: Dict
    ) -> bool:
        """
        ...
        try:
            ...
            for country_data in assessment_data.get("countries", []):
                ...
                # Save country score
                country_score_id = self.save_country_score(
                    assessment_id,
                    {
                        ...
                        "confidence_interval_lower": country_data.get(
                            "confidence_interval", [None, None]
                        )[0],
                        "confidence_interval_upper": country_data.get(
                            "confidence_interval", [None, None]
                        )[1],
                        "data_quality_grade": country_data.get("data_quality_grade"),
                        "data_completeness_pct": country_data.get(
                            "data_completeness_pct"
                        ),
                        "expert_validation_status": country_data.get(
                ...
                if country_score_id:
                    ...
                    if "pillar_scores" in country_data:
                        self.save_pillar_scores(
                            country_score_id, country_data["pillar_scores"]
                        )

                    # Save indicators
                    if "indicators" in country_data:
                        self.save_indicator_values(
                            country_score_id, country_data["indicators"]
                        )
            ...
            logger.info(
                f"Saved complete assessment data: {success_count}/{total_operations} countries"
            )
