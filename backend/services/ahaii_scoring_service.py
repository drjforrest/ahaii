...
class AHAIIScoringService:
    ...
    def calculate_indicator_score(
        self, value: float, indicator: IndicatorDefinition
    ) -> float:
        ...
        if indicator.calculation_method == "linear":
            ...
            score = min(
                100.0,
                (math.log(1 + value) / math.log(1 + indicator.target_value)) * 100.0,
            )
            ...
            score = (
                ...
                else (value / indicator.target_value) * 100.0
            )
    ...
    def calculate_sub_pillar_score(
        self, indicators_data: Dict[str, float], sub_pillar: str, pillar: str
    ) -> Tuple[float, Dict]:
        """Calculate weighted score for a sub-pillar"""

        relevant_indicators = {
            name: definition
            for name, definition in self.indicator_definitions.items()
    ...
    def calculate_pillar_score(
        self, indicators_data: Dict[str, float], pillar: str
    ) -> Tuple[float, Dict]:
    ...
    def calculate_ahaii_score(
        self, indicators_data: Dict[str, float], country_id: str, assessment_year: int
    ) -> Dict:
        """
        ...
        for pillar in pillars:
            pillar_score, pillar_breakdown = self.calculate_pillar_score(
                indicators_data, pillar
            )
            ...
            pillar_indicators_with_data = sum(
                1
                for sub_pillar_data in pillar_breakdown.values()
        ...
        data_completeness_percentage = (
            data_availability_count / total_indicators_count
        ) * 100
        confidence_score = self._calculate_confidence_score(
            data_completeness_percentage, pillar_scores
        )

        # Generate tier justification
        tier_justification = self._generate_tier_justification(
            total_score, pillar_scores, readiness_tier
        )
        ...
        return {
            ...
            "physical_infrastructure_score": pillar_scores[
                "physical_infrastructure_score"
            ],
            "regulatory_infrastructure_score": pillar_scores[
            ...
            "indicators_missing": [
                name
                for name in self.indicator_definitions.keys()
    ...
    def _calculate_confidence_score(
        ...
        if len(pillar_values) > 1:
            ...
            consistency_confidence = max(
                0.5, 1.0 - (pillar_std / 50.0)
            )  # Penalize high variation
        else:
            consistency_confidence = 1.0

        # Combined confidence score
        confidence_score = (completeness_confidence * 0.7) + (
            consistency_confidence * 0.3
        )

        return round(min(1.0, confidence_score), 2)

    def _generate_tier_justification(
        self, total_score: float, pillar_scores: Dict, tier: int
    ) -> str:
        ...
        if tier == 1:
            ...
            return (
                f"Tier 1 (Implementation Ready): Total score of {total_score} with strong "
            ...
            return (
                f"Tier 2 (Foundation Building): Total score of {total_score} demonstrates solid "
            ...
            return (
                f"Tier 3 (Readiness Development): Total score of {total_score} indicates "
    ...
    def _identify_key_strengths(self, pillar_scores: Dict) -> List[str]:
        ...
        for pillar, score in pillar_scores.items():
            ...
            if score >= 75.0:
                strengths.append(
                    f"Excellent {pillar_name} infrastructure ({score:.1f}/100)"
                )
            elif score >= 60.0:
        ...
        if not strengths:
            ...
            strengths.append(
                f"Relative strength in {pillar_name} ({best_score:.1f}/100)"
            )
        ...
        for pillar, score in sorted_pillars:
            ...
            if score < 40.0:
                improvement_areas.append(
                    f"Critical gap in {pillar_name} ({score:.1f}/100) - requires immediate attention"
                )
            elif score < 60.0:
                improvement_areas.append(
                    f"{pillar_name} development needed ({score:.1f}/100)"
                )
...
def get_indicator_definitions_summary() -> Dict:
    ...
    for pillar in [
        ...
        pillar_indicators = [
            name
            for name, defn in service.indicator_definitions.items()
