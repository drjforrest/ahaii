...
ISO3_TO_IMAGE_NAME = {
    'AGO': 'angola', 'BEN': 'benin', 'BWA': 'botswana', 'CIV': 'cote-divoire',
    'EGY': 'egypt', 'ETH': 'ethiopia', 'GHA': 'ghana', 'KEN': 'kenya',
    'MUS': 'mauritius', 'NGA': 'nigeria', 'RWA': 'rwanda', 'SEN': 'senegal',
    'SYC': 'seychelles', 'ZAF': 'south-africa', 'TUN': 'tunisia', 'ZMB': 'zambia',
    'DZA': 'algeria', 'MAR': 'morocco', 'LBY': 'libya', 'SUD': 'sudan',
    'TZA': 'tanzania', 'UGA': 'uganda', 'MOZ': 'mozambique', 'MDG': 'madagascar'
...
ISO3_TO_ISO2 = {
    ...
    'ZMB': 'zm', 'DZA': 'dz', 'MAR': 'ma', 'LBY': 'ly', 'SUD': 'sd',
    'TZA': 'tz', 'UGA': 'ug', 'MOZ': 'mz', 'MDG': 'mg'
}
...
async def calculate_featured_countries(countries: List[Dict], limit: int = 8) -> List[Dict]:
    ...
    for country in countries:
        ...
        feature_score = (
            ...
            {'leader': 100, 'ready': 80, 'building': 60, 'emerging': 40}.get(tier, 0) * 0.2  # 20% tier
        )
        
        featured_scores.append({
            'country': country,
...
async def get_all_countries_with_scores(
    include_estimated: bool = Query(True, description="Include countries with estimated scores"),
    ...
    try:
        ...
        for country in countries_response.data:
            ...
            # Get AHAII score
            score_response = supabase.table("ahaii_scores")\
                .select("*")\
            ...
            activity_response = supabase.table("infrastructure_intelligence")\
            ...
            last_update_response = supabase.table("infrastructure_intelligence")\
                .select("created_at")\
                .eq("country_id", country_id)\
                .order("created_at", desc=True)\
                .limit(1)\
            ...
            country_data = {
                ...
                "healthcare_spending_percent_gdp": country.get("healthcare_spending_percent_gdp"),
            ...
            if not include_estimated or (ahaii_score and ahaii_score.get("confidence", 0) >= min_confidence):
...
async def get_featured_countries(
    ...
    try:
        ...
        countries_with_scores = await get_all_countries_with_scores(db_session=db_session)
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching featured countries: {str(e)}")
...
async def get_countries_by_region(
    ...
    try:
        ...
        results.sort(key=lambda x: x["ahaii_score"]["total_score"] if x["ahaii_score"] else 0, reverse=True)
...
async def get_country_details(
    ...
    try:
        ...
        country_response = supabase.table("countries")\
            .select("*")\
        ...
        signals_response = supabase.table("infrastructure_intelligence")\
            .select("*")\
            .eq("country_id", country_id)\
            .order("created_at", desc=True)\
            .limit(20)\
        ...
        indicators_response = supabase.table("infrastructure_indicators")\
            .select("*")\
            .eq("country_id", country_id)\
            .order("created_at", desc=True)\
            .execute()
        ...
        orgs_response = supabase.table("health_ai_organizations")\
            .select("*")\
            .eq("country_id", country_id)\
            .limit(10)\
            .execute()
...
async def get_countries_with_recent_activity(
    ...
    try:
        ...
        sorted_countries = sorted(country_activity.items(), key=lambda x: x[1], reverse=True)[:limit]
        ...
        for country_id, activity_count in sorted_countries:
            ...
            latest_response = supabase.table("infrastructure_intelligence")\
                .select("created_at")\
                .eq("country_id", country_id)\
                .order("created_at", desc=True)\
                .limit(1)\
...
async def get_country_statistics(db_session=Depends(get_database_session)):
    ...
    try:
        # Get total countries
        countries_response = supabase.table("countries").select("id", count="exact").execute()
        total_countries = countries_response.count or 0
        
        # Get countries with scores
        scores_response = supabase.table("ahaii_scores").select("id", count="exact").execute()
        countries_with_scores = scores_response.count or 0
        
        # Get countries with recent data (last 30 days)
        cutoff_date = (datetime.now() - timedelta(days=30)).isoformat()
        recent_data_response = supabase.table("infrastructure_intelligence")\
            .select("country_id", count="exact")\
            .gte("created_at", cutoff_date)\
            .execute()
        
        # Count unique countries with recent data
        unique_countries_recent = len(set(item["country_id"] for item in recent_data_response.data or []))
        
        # Get all scores for average calculation
        all_scores_response = supabase.table("ahaii_scores")\
            .select("total_score")\
            .execute()
        
        scores = [item["total_score"] for item in all_scores_response.data or [] if item.get("total_score")]
        avg_score = sum(scores) / len(scores) if scores else None
        
        # Get tier distribution
        tier_response = supabase.table("ahaii_scores")\
        ...
        return {
            ...
            "coverage_percentage": (countries_with_scores / total_countries * 100) if total_countries > 0 else 0,
            "data_freshness_percentage": (unique_countries_recent / total_countries * 100) if total_countries > 0 else 0
        }
...
async def get_regional_overview(db_session=Depends(get_database_session)):
    ...
    try:
        ...
        # Get all scores
        scores_response = supabase.table("ahaii_scores")\
            .select("*")\
            .execute()
        ...
        for region_data in regional_stats.values():
            ...
            regions.append({
                ...
                "coverage_percentage": (len(region_data["scores"]) / region_data["country_count"] * 100) if region_data["country_count"] > 0 else 0
            })
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching regional overview: {str(e)}")
