...
async def get_data_collection_status():
    ...
    try:
        ...
        if hasattr(manager, 'pipeline_results') and 'data_collection' in manager.pipeline_results:
            ...
            wb_status = collection_results.get('world_bank', {}).get('status', 'not_started')
            policy_status = collection_results.get('policy', {}).get('status', 'not_started')
            ecosystem_status = collection_results.get('ecosystem', {}).get('status', 'not_started')
            
            if wb_status == 'success':
                successful_components += 1
                total_data_points += collection_results['world_bank'].get('data_points', 0)
            ...
            if ecosystem_status == 'success':
                successful_components += 1
                total_data_points += collection_results['ecosystem'].get('organizations_mapped', 0)
...
async def calculate_ahaii_scores(
    ...
    try:
        ...
        if not hasattr(manager, 'pipeline_results') or 'data_collection' not in manager.pipeline_results:
            raise HTTPException(
                status_code=400, 
...
async def get_ahaii_scores(
    ...
    try:
        ...
        if (not hasattr(manager, 'pipeline_results') or 
            'scoring' not in manager.pipeline_results or
            'enhanced_scores' not in manager.pipeline_results['scoring']):
            raise HTTPException(
                status_code=404,
        ...
        if scoring_results.get('status') != 'success':
            raise HTTPException(
                status_code=500,
                detail="Score calculation failed or incomplete"
            )
        ...
        if format == "summary":
            ...
            for result in enhanced_results:
                country_scores.append(AHAIICountryScore(
            ...
            regional_average = sum(score.total_score for score in country_scores) / len(country_scores)
...
async def run_expert_validation(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if (not hasattr(manager, 'pipeline_results') or
            'data_collection' not in manager.pipeline_results or
            'scoring' not in manager.pipeline_results):
            raise HTTPException(
                status_code=400,
        ...
        raise HTTPException(status_code=500, detail=f"Expert validation failed: {str(e)}")
...
async def generate_final_report(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        for phase in required_phases:
            if (not hasattr(manager, 'pipeline_results') or
                phase not in manager.pipeline_results):
                raise HTTPException(
                    status_code=400,
        ...
        def run_reporting():
            ...
            return manager.run_reporting_phase(collection_results, scoring_results, validation_results)
...
@router.post("/run-complete-assessment", summary="Run complete AHAII assessment pipeline")
async def run_complete_assessment(
    background_tasks: BackgroundTasks,
    request: AHAIIAssessmentRequest
):
    """
    ...
    logger.info(f"⚙️ Components: Policy={request.include_policy_indicators}, Ecosystem={request.include_ecosystem_mapping}, Validation={request.include_expert_validation}, Report={request.generate_report}")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Complete assessment failed: {str(e)}")
