...
_analytics_cache = {"summary": None, "full": None, "last_updated": None}
...
async def _refresh_cache():
    ...
    try:
        ...
        _analytics_cache.update(
            {"summary": summary, "full": full_data, "last_updated": datetime.now()}
        )
...
async def get_dashboard_summary(
    background_tasks: BackgroundTasks, force_refresh: bool = False
) -> Dict[str, Any]:
    """
...
async def get_time_series(days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get time series: {str(e)}"
        )
...
async def get_domain_breakdown() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get domain breakdown: {str(e)}"
        )
...
async def get_quality_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "verification_rate": (
                round((metrics.verified_records / total_records) * 100, 1)
                if total_records > 0
                else 0
            ),
            "high_confidence_rate": (
                round((metrics.high_confidence_records / total_records) * 100, 1)
                if total_records > 0
                else 0
            ),
            "peer_reviewed_rate": (
                round((metrics.peer_reviewed_sources / total_records) * 100, 1)
                if total_records > 0
                else 0
            ),
    ...
    try:
        ...
        return {
            ...
            "success_rate": (
                round(
                    (
                        metrics.successful_pipeline_runs
                        / (
                            metrics.successful_pipeline_runs
                ...
                if (metrics.successful_pipeline_runs + metrics.failed_pipeline_runs) > 0
                else 100
            ),
        }
...
async def get_country_coverage() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "coverage_rate": (
                round(
                    (len(metrics.country_coverage) / metrics.total_countries) * 100, 1
                )
            ...
            "top_countries": (
                dict(
                    list(
                        sorted(
                            metrics.country_coverage.items(),
                            key=lambda x: x[1],
                            reverse=True,
                        )
                    )[:10]
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get country coverage: {str(e)}"
        )
...
async def refresh_analytics(background_tasks: BackgroundTasks) -> Dict[str, str]:
    ...
    try:
        ...
        return {
            ...
            "message": "Analytics cache refresh started in background",
        }

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to trigger refresh: {str(e)}"
        )
...
async def get_analytics_health() -> Dict[str, Any]:
    ...
    try:
        ...
        cache_status = (
            "fresh"
            if not _is_cache_expired() and _analytics_cache["summary"]
            else "stale"
        )
        ...
        return {
            ...
            "last_cache_update": (
                _analytics_cache["last_updated"].isoformat()
                if _analytics_cache["last_updated"]
                else None
            ),
