...
class AHAIIAssessment(Base):
    ...
    country_scores = relationship(
        "AHAIICountryScore", back_populates="assessment", cascade="all, delete-orphan"
    )
    data_collection_runs = relationship(
        "DataCollectionRun", back_populates="assessment", cascade="all, delete-orphan"
    )
...
class AHAIICountryScore(Base):
    ...
    assessment_id = Column(
        String(50), ForeignKey("ahaii_assessments.assessment_id"), nullable=False
    )
    ...
    pillar_scores = relationship(
        "AHAIIPillarScore", back_populates="country_score", cascade="all, delete-orphan"
    )
...
class AHAIIPillarScore(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    country_score_id = Column(
        Integer, ForeignKey("ahaii_country_scores.id"), nullable=False
    )
...
def get_latest_assessment(db_session) -> Optional[AHAIIAssessment]:
    ...
    return (
        db_session.query(AHAIIAssessment)
        .order_by(AHAIIAssessment.assessment_date.desc())
        .first()
    )


def get_country_scores_by_assessment(
    db_session, assessment_id: str
) -> list[AHAIICountryScore]:
    """Get all country scores for a specific assessment"""
    return (
        db_session.query(AHAIICountryScore)
        .filter(AHAIICountryScore.assessment_id == assessment_id)
        .order_by(AHAIICountryScore.total_score.desc())
        .all()
    )


def get_pillar_scores_by_country(
    db_session, country_score_id: int
) -> list[AHAIIPillarScore]:
    """Get all pillar scores for a specific country"""
    return (
        db_session.query(AHAIIPillarScore)
        .filter(AHAIIPillarScore.country_score_id == country_score_id)
        .all()
    )
...
def get_indicators_by_country(
    db_session, country_score_id: int
) -> list[AHAIIIndicatorValue]:
    """Get all indicators for a specific country"""
    return (
        db_session.query(AHAIIIndicatorValue)
        .filter(AHAIIIndicatorValue.country_score_id == country_score_id)
        .order_by(AHAIIIndicatorValue.pillar_name, AHAIIIndicatorValue.indicator_name)
        .all()
    )


def get_assessment_history(
    db_session, country_code: str, limit: int = 10
) -> list[AHAIICountryScore]:
    """Get assessment history for a specific country"""
    return (
        db_session.query(AHAIICountryScore)
        .filter(AHAIICountryScore.country_code == country_code)
        .order_by(AHAIICountryScore.calculation_date.desc())
        .limit(limit)
        .all()
    )
...
def get_regional_rankings(db_session, assessment_id: str) -> list[tuple]:
    ...
    return (
        ...
        .filter(AHAIICountryScore.assessment_id == assessment_id)
        .order_by(AHAIICountryScore.regional_rank)
        .all()
    )
